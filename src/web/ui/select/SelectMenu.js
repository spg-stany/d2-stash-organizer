...
import { chainedRef, searchRegex } from 'utils';
...
export default function SelectMenu(props) {
  ...
  const { options, enableSearch, search } = React.useMemo(() => {
    ...
    const search = (searchTerm && enableSearch ? searchRegex(searchTerm, searchMiddle) : null);
    const flattenOptions = (list, groups = ['_g']) => {
      const result = [];
      list.forEach(opt => {
        if (opt.options) {
          ...
          if (sub.length) {
            result.push({ id: ngroups.join("."), depth: groups.length - 1, title: opt.title }, ...sub);
          }
        } else if (!search || searchFunc(opt, search)) {
          result.push({ id: optionId(opt), depth: groups.length - 1, disabled: isDisabled(opt), value: opt });
        }
      });
      return result;
    };
